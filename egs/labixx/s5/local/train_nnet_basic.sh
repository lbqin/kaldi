...
cat $data/feats.scp | local/shuffle_list.pl --srand ${seed:-777} > $dir/intrain.scp
cp $data_cv/feats.scp $dir/incv.scp
cat $odata/feats.scp |awk -v lst=$data/feats.scp 'BEGIN{ while (getline < lst) w[$1] = 1}{if (w[$1]) print}' | local/shuffle_list.pl --srand ${seed:-777} > $dir/train.scp
...
if [ "$copy_feats" == "true" ]; then
  ...
  local/nnet/copy_feats.sh $dir/train.scp_non_local $tmpdir $dir/train.scp
  tmpdir2=$(mktemp -d); mv $dir/intrain.scp $dir/intrain.scp_non_local
  local/nnet/copy_feats.sh $dir/intrain.scp_non_local $tmpdir2 $dir/intrain.scp
  #remove data on exit...
...
if [ "$feature_transform" != "" ]; then
  ...
  local/nnet/gen_splice.py --fea-dim=$feat_dim --splice=$splice_lr --splice-step=$splice_step > $feature_transform
  ...
  case $feat_type in
    ...
    traps)
      ...
      local/nnet/gen_hamm_mat.py --fea-dim=$feat_dim --splice=$splice_lr > $dir/hamm.mat
      local/nnet/gen_dct_mat.py --fea-dim=$feat_dim --splice=$splice_lr --dct-basis=$traps_dct_basis > $dir/dct.mat
...
if [ ! -z "$mlp_init" ]; then
  ...
  local/nnet/init_nnet.sh --model_size $model_size --hid_layers $hid_layers \
...
#         echo " $mlp_init"
#         local/nnet/gen_mlp_init.py --dim=${num_fea}:${num_tgt} \
...
#         echo " $mlp_init"
#         local/nnet/gen_mlp_init.py --dim=${num_fea}:${num_hid}:${num_tgt} \
...
#         echo " $mlp_init"
#         local/nnet/gen_mlp_init.py --dim=${dim_arg} --gauss --negbias --seed=$seed --activationOutput=$activationOutput $mlpOption > $mlp_init
#         ;;
#       *)
...
#         echo " $mlp_init"
#         local/nnet/gen_mlp_init.py --dim=${num_fea}:${num_hid}:${num_bn}:${num_hid}:${num_tgt} --gauss --negbias --seed=$seed --activationOutput=$activationOutput --linBNdim=$num_bn $mlpOption > $mlp_init
#         ;;
...
#         echo " $mlp_init"
#         local/nnet/gen_mlp_init.py --dim=${dim_arg} --gauss --negbias --seed=$seed --activationOutput=$activationOutput --linBNdim=$num_bn $mlpOption > $mlp_init
#         ;;
#       *)
#         echo "Unsupported number of hidden layers $hid_layers"
#         exit 1;
...
source local/nnet/train_nnet_basic_scheduler.sh
